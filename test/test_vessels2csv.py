...
from anat2vessels.vessels2csv import main, out_list_to_df, ray_main
...
class TestRayMain:
    ...
    @patch("anat2vessels.vessels2csv.cpu_count")
    @patch("anat2vessels.vessels2csv.ray")
    @patch("anat2vessels.vessels2csv.out_list_to_df")
    @patch("anat2vessels.vessels2csv.extract_features")
    @patch("anat2vessels.vessels2csv.os.listdir")
    def test_processes_matching_files(
        self, mock_listdir, mock_extract, mock_out, mock_ray, mock_cpu
    ):
        mock_cpu.return_value = 4
        mock_listdir.return_value = ["sub-01_seg.nii.gz", "sub-02_seg.nii.gz"]
        mock_extract.return_value = self.MOCK_FEATURES
        future = object()
        mock_ray.remote.return_value.remote.return_value = future
        mock_ray.get.return_value = self.MOCK_FEATURES
        mock_out.return_value = pd.DataFrame()
        args = MockArgs(use_ray=True)
        with patch.object(pd.DataFrame, "to_csv") as mock_to_csv:
            ray_main(args)
        mock_ray.init.assert_called_once_with(num_cpus=3)
        assert mock_ray.remote.call_count == 1
        assert mock_ray.remote.return_value.remote.call_count == 2
    ...
    @patch("anat2vessels.vessels2csv.extract_features")
    @patch("anat2vessels.vessels2csv.os.listdir")
    def test_subject_id_in_data_list(
        self, mock_listdir, mock_extract, mock_out, mock_ray, mock_cpu
    ):
        mock_cpu.return_value = 4
        mock_listdir.return_value = ["sub-01_seg.nii.gz"]
    ...
    def test_skips_non_nifti(
        ...
        with patch.object(pd.DataFrame, "to_csv"):
            ray_main(args)
        assert mock_ray.remote.return_value.remote.call_count == 1
    ...
    @patch("anat2vessels.vessels2csv.extract_features")
    @patch("anat2vessels.vessels2csv.os.listdir")
    def test_handles_ray_get_error(
        self, mock_listdir, mock_extract, mock_out, mock_ray, mock_cpu
    ):
        mock_cpu.return_value = 4
        mock_listdir.return_value = ["sub-01_seg.nii.gz", "sub-02_seg.nii.gz"]
        ...
        future = object()
        mock_ray.remote.return_value.remote.return_value = future
        mock_ray.get.side_effect = [self.MOCK_FEATURES, ValueError("ray failed")]
        mock_out.return_value = pd.DataFrame({"sub_id": ["sub-01"]})
        args = MockArgs(use_ray=True)
        with patch.object(pd.DataFrame, "to_csv"):
            ray_main(args)
    ...
    def test_no_matching_files(
        self, mock_listdir, mock_extract, mock_out, mock_ray, mock_cpu, capsys
    ):
        mock_cpu.return_value = 4
        mock_listdir.return_value = ["readme.txt", "data.csv"]
        args = MockArgs(use_ray=True)
    ...
    @patch("anat2vessels.vessels2csv.extract_features")
    @patch("anat2vessels.vessels2csv.os.listdir")
    def test_writes_to_correct_output_path(
        self, mock_listdir, mock_extract, mock_out, mock_ray, mock_cpu
    ):
        mock_cpu.return_value = 4
        mock_listdir.return_value = ["sub-01_seg.nii.gz"]
        ...
        mock_ray.get.return_value = self.MOCK_FEATURES
        mock_out.return_value = pd.DataFrame()
        args = MockArgs(output_path="/custom/path/output.csv", use_ray=True)
        with patch.object(pd.DataFrame, "to_csv") as mock_to_csv:
            ray_main(args)
        mock_to_csv.assert_called_once_with("/custom/path/output.csv", index=False)
