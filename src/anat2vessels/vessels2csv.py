...
def out_list_to_df(out_list):
    ...
    for item in out_list:
        try:
            ...
            if item["radius_list"]:
                out["mean_radius"] = float(
                    sum(item["radius_list"]) / len(item["radius_list"])
                )
            ...
            tortiousities = np.array(
                [branch["tortuosity"] for branch in item["branch_list"]]
            )
            ...
            branch_lengths = [
                float(branch["full_path"]) for branch in item["branch_list"]
            ]
            out["branch_list"] = [
                {
                    "full_path": float(branch["full_path"]),
                    "straight_path": float(branch["straight_path"]),
                    "tortuosity": float(branch["tortuosity"]),
                }
                for branch in item["branch_list"]
            ]
...
def run_feature_extraction(input_dir, output_path, use_ray=True):
    ...
    if ray is not None and use_ray:
        ray_main(args)
    else:
        main(args)
...
def run():
    ...
    run_feature_extraction(args.input_dir, args.output_path, args.use_ray)
